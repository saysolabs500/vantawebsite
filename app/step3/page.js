...
function Step3Content() {
  ...
  useEffect(() => {
    ...
    if (!hash) {
      setError('No verification data. Complete the Linkvertise step from the beginning.');
      return;
    }
    fetch(`/api/verify-hash?hash=${encodeURIComponent(hash)}&debug=1`)
      ...
      .then((data) => {
        ...
        setError(data.error || 'Verification failed. Complete the Linkvertise ad fully.');
      })
