...
function Step2Content() {
  ...
  useEffect(() => {
    ...
    if (!hash) {
      setStatus('No verification data. Use "Get Key" from the launcher and complete the Linkvertise step fully.');
      return;
    }
    fetch(`/api/verify-hash?hash=${encodeURIComponent(hash)}&debug=1`)
      .then((r) => r.json())
      .then((data) => {
        if (data.valid) {
          setOk(true);
          ...
          setStatus(data.error || 'Verification failed. Complete the Linkvertise ad fully, then wait for redirect.');
        }
      })
      .catch(() => setStatus('Verification failed. Check your connection.'));
  }, [searchParams]);
